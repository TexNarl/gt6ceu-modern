 ...
 * Source: gregtech/worldgen/WorldgenRiver.java
 * Purpose: Replace river water blocks with GT river fluid blocks.
 * Status: PORTED
 * Notes: Applies only in river biomes and uses sea level as upper bound.
 */
...
public class GtWorldgenRiver implements GtChunkWorldgen {
    private final String id;

    public GtWorldgenRiver(String id) {
        this.id = id;
    }
            ...
            for (int dz = 0; dz < 16; dz++) {
                ...
                for (int y = seaLevel; y >= level.getMinBuildHeight(); y--) {
                    ...
                    if (level.getBlockState(scan).is(Blocks.WATER)) {
                        level.setBlock(scan, GTBlocks.RIVER_WATER.get().defaultBlockState(), 2);

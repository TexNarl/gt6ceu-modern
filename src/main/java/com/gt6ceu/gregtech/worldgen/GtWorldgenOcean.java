 ...
 * Source: gregtech/worldgen/WorldgenOcean.java
 * Purpose: Replace ocean water blocks with GT ocean fluid blocks.
 * Status: PORTED
 * Notes: Applies only in ocean biomes and uses sea level as upper bound.
 */
...
public class GtWorldgenOcean implements GtChunkWorldgen {
    private final String id;

    public GtWorldgenOcean(String id) {
        this.id = id;
    }
            ...
            for (int dz = 0; dz < 16; dz++) {
                ...
                for (int y = seaLevel; y >= level.getMinBuildHeight(); y--) {
                    ...
                    if (level.getBlockState(scan).is(Blocks.WATER)) {
                        level.setBlock(scan, GTBlocks.OCEAN_WATER.get().defaultBlockState(), 2);
